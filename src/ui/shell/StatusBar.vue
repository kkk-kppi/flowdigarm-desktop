<template>
  <footer class="status-bar" data-testid="statusbar">
    ...
    <div class="status-right">
      ...
      <button type="button" data-testid="status-grid" :aria-pressed="appStore.showGrid" aria-label="切换网格" title="切换网格。显示或隐藏画布网格，不进入撤销历史。" @click="appStore.toggleGrid()">网格 {{ appStore.showGrid ? '开' : '关' }}</button>
      <button type="button" data-testid="status-snap" :aria-pressed="appStore.snapToGrid" aria-label="切换对齐吸附" title="切换对齐吸附。控制图元移动时的网格吸附，不进入撤销历史。" @click="appStore.toggleSnap()">对齐 {{ appStore.snapToGrid ? '开' : '关' }}</button>
