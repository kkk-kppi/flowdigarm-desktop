import { createCanvasFixture, expect, openCleanEditor, snapshot, test, waitForX6Cell, x6Cell } from './fixtures'
...
test('renders ports, edge endpoints/vertices, connector visuals, and group/container commands', async ({ page }) => {
  ...
  await expect(async () => {
    await (await waitForX6Cell(page, ids.orthogonal, state.revision)).locator('path').first().click({ force: true })
  ...
  const orthPath = await (await waitForX6Cell(page, ids.orthogonal, vertexState.revision)).locator('path').first().getAttribute('d')
  const curvePath = await (await waitForX6Cell(page, ids.curved, vertexState.revision)).locator('path').first().getAttribute('d')
  expect(orthPath).not.toBe(curvePath)
  await (await waitForX6Cell(page, ids.first, vertexState.revision)).hover({ force: true })
  ...
  const reconnectPort = await (await waitForX6Cell(page, ids.third, vertexState.revision)).locator('[port="left"]').boundingBox()
  ...
  await (await waitForX6Cell(page, ids.first, state.revision)).click({ force: true })
  await (await waitForX6Cell(page, ids.second, state.revision)).click({ force: true, modifiers: ['Shift'] })
  await page.getByRole('menuitem', { name: '格式' }).click()
